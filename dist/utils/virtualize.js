...
function computeVisibleRange({
  ...
  const clampedScroll = clamp(scrollOffset, 0, Math.max(0, totalSize - viewportSize));
...
function computeVisibleRangeVariable({
  ...
  let firstVisible = 0;
  while (firstVisible < itemCount - 1 && offsets[firstVisible] + itemSizes[firstVisible] <= clampedScroll) {
    firstVisible += 1;
  }
...
export { computeVisibleRange, computeVisibleRangeVariable };
