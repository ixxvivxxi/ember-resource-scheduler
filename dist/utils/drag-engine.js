...
function createDragEngine({
  ...
  function assertDragging(method) {
    if (state !== DRAGGING) {
      throw new Error(`drag-engine: ${method}() called with no drag in progress`);
    }
  }
  function toResult(candidate) {
    return {
      ...candidate,
      valid: canPlace(candidate)
    };
  }
  return {
    get isDragging() {
    ...
    start(candidate) {
      if (state === DRAGGING) {
        throw new Error('drag-engine: start() called while a drag is already in progress');
      }
      state = DRAGGING;
      gesture = {
        ...candidate
...
export { createDragEngine };
//# sourceMappingURL=drag-engine.js.map
