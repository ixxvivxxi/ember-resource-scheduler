 ...
 */
function buildLaneLayout(resources, {
  laneSize,
  getLaneCount = resource => resource.laneCount ?? 1
}) {
  const rows = [];
  let cursor = 0;
  ...
  function locate(offset) {
    ...
    const row = rows.find(candidate => clamped < candidate.offset + candidate.size) ?? rows[rows.length - 1];
    const lane = clamp(Math.floor((clamped - row.offset) / laneSize), 0, row.laneCount - 1);
    return {
...
export { buildLaneLayout };
//# sourceMappingURL=lane-layout.js.map
