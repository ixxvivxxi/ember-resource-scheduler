...
import ResourceTimelineHeader from './resource-timeline/header.gjs';
import ResourceTimelineNameCell from './resource-timeline/name-cell.gjs';
import ResourceTimelineResourceRow from './resource-timeline/resource-row.gjs';
import ResourceTimelineSummaryRow from './resource-timeline/summary-row.gjs';
 ...
 */
export default class ResourceTimeline extends Component {
  @tracked scrollLeft = 0;
  @tracked scrollTop = 0;
  @tracked viewportWidth = 0;
  @tracked viewportHeight = 0;
  @tracked dragPreview = null;
  ...
  get placeColWidth() {
    return this.args.laneLabelFor
      ? (this.args.placeColWidth ?? DEFAULT_PLACE_COL_WIDTH)
      : 0;
  }
  ...
  @cached
  get laneLayout() {
    return buildLaneLayout(this.resources, { laneSize: this.laneHeight });
  }
  ...
  get pinnedLaneLayout() {
    return buildLaneLayout(this.pinnedResources, { laneSize: this.laneHeight });
  }
  ...
  get visibleRowRange() {
    return computeVisibleRangeVariable({
      ...
      scrollOffset:
        this.scrollTop -
        this.headerHeight -
        this.extraHeaderHeight -
        this.pinnedBandHeight,
  ...
  get cornerExtraStyle() {
    return htmlSafe(
      `height: ${this.extraHeaderHeight}px; top: ${this.headerHeight}px;`,
    );
  }
  ...
  get pinnedBandStyle() {
    return htmlSafe(
      `top: ${this.headerHeight + this.extraHeaderHeight}px; height: ${this.pinnedBandHeight}px;`,
    );
  }
  ...
  placeRowsFor = (rows) => {
    ...
    return rows.map((row) => {
      const laneCount = row.isSummary
        ? 0
        : Math.max(1, row.resource.laneCount ?? 1);
      return {
        ...
        lanes: Array.from({ length: laneCount }, (_, laneIndex) => ({
          ...
          style: htmlSafe(
            `top: ${laneIndex * laneHeight}px; height: ${laneHeight}px;`,
          ),
        })),
      };
    });
  ...
  <template>
    {{#let
      (component
        ...
        activatable=@onResourceActivate
        onActivate=this.activateResource
        hasCustomName=(has-block "name")
      )
      (component
      ...
      as |NameCell LaneRow SummaryRow|
    }}
      <div class="rs-resource-timeline" ...attributes>
        <div
          class="rs-resource-timeline__viewport"
          {{scrollWindow onChange=this.handleScrollChange}}
        >
          <div class="rs-resource-timeline__body">
            <div class="rs-resource-timeline__name-column">
              <div
                class="rs-resource-timeline__corner"
                style={{this.cornerStyle}}
              ></div>
              {{#if this.hasExtraHeader}}
                <div
                  class="rs-resource-timeline__corner-extra"
                  style={{this.cornerExtraStyle}}
                >
              ...
              {{#if this.hasPinnedRows}}
                <div
                  class="rs-resource-timeline__pinned-names"
                  style={{this.pinnedBandStyle}}
                >
            ...
            {{#if @laneLabelFor}}
              <div class="rs-resource-timeline__place-column">
                <div
                  class="rs-resource-timeline__corner"
                  style={{this.placeCornerStyle}}
                ></div>
                {{#if this.hasPinnedRows}}
                  <div
                    class="rs-resource-timeline__pinned-places"
                    style={{this.pinnedBandStyle}}
                  >
                    {{#each this.pinnedPlaceRows key="resource.id" as |row|}}
                      <div
                        class="rs-resource-timeline__place-cell-group"
                        style={{row.style}}
                      >
                        {{#each row.lanes key="laneIndex" as |lane|}}
                          <div
                            class="rs-resource-timeline__place-cell"
                            style={{lane.style}}
                          >{{lane.label}}</div>
                        {{/each}}
                      </div>
            ...
            {{/if}}
            <div
              class="rs-resource-timeline__lanes-column"
              data-rs-lanes-content
              style={{this.lanesColumnStyle}}
            >
              <ResourceTimelineHeader
                @axis={{@axis}}
                @headerHeight={{this.headerHeight}}
                @visibleColumnRange={{this.visibleColumnRange}}
                @unitClassFor={{@unitClassFor}}
                @unitLabelFor={{@unitLabelFor}}
              />
              {{#if this.hasExtraHeader}}
                <div
                  class="rs-resource-timeline__header-extra-row"
                  style={{this.headerExtraRowStyle}}
                >
                  {{yield
                    (hash axis=@axis visibleColumnRange=this.visibleColumnRange)
                    to="headerExtraRow"
                  }}
                </div>
              {{/if}}
              {{#if this.hasPinnedRows}}
                <div
                  class="rs-resource-timeline__pinned-rows"
                  data-rs-pinned-rows
                  style={{this.pinnedBandStyle}}
                >
                  {{#each this.pinnedRows key="resource.id" as |row|}}
