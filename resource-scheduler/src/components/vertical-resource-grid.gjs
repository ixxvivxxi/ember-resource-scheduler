...
import VerticalResourceGridTimeHeader from './vertical-resource-grid/time-header.gjs';
import VerticalResourceGridResourceColumn from './vertical-resource-grid/resource-column.gjs';
 ...
 */
export default class VerticalResourceGrid extends Component {
  ...
  @cached
  get laneLayout() {
    return buildLaneLayout(this.resources, { laneSize: this.laneWidth });
  }
  ...
  get visibleColumns() {
    const { startIndex, endIndex } = this.visibleColumnRange;
  ...
  <template>
    <div class="rs-vertical-resource-grid" ...attributes>
      <div
        class="rs-vertical-resource-grid__viewport"
      ...
      >
        <div class="rs-vertical-resource-grid__body">
          <div class="rs-vertical-resource-grid__header-row">
            <div
              class="rs-vertical-resource-grid__corner"
            ...
            {{#each this.visibleColumns key="resource.id" as |column|}}
              <div
                class="rs-vertical-resource-grid__resource-header-cell"
                style={{column.style}}
              >
                <div
                  class="rs-vertical-resource-grid__resource-header-label"
                >{{column.resource.label}}</div>
                {{#if column.resource.sublabel}}
                  <div
                    class="rs-vertical-resource-grid__resource-header-sublabel"
                  >{{column.resource.sublabel}}</div>
                {{/if}}
              </div>
            {{/each}}
          ...
          <div
            class="rs-vertical-resource-grid__lanes-row"
            data-rs-lanes-content
            style={{this.lanesRowStyle}}
          >
            <VerticalResourceGridTimeHeader
              @axis={{@axis}}
              @timeColWidth={{this.timeColWidth}}
              @visibleUnitRange={{this.visibleUnitRange}}
              @unitClassFor={{@unitClassFor}}
              @unitLabelFor={{@unitLabelFor}}
            />
            <div
              class="rs-vertical-resource-grid__columns-content"
              data-rs-rows-content
            >
              {{#if this.leadingColumnSpacerSize}}
                <div style={{this.leadingColumnSpacerStyle}}></div>
              {{/if}}
