 ...
 */
export function lanesContentElementFor(element) {
 ...
 */
export function rowsContentElementFor(element) {
  ...
  return lanesContentElement
    ? lanesContentElement.querySelector('[data-rs-rows-content]')
    : null;
}

export function pinnedRowsElementFor(element) {
  const lanesContentElement = lanesContentElementFor(element);
  return lanesContentElement
    ? lanesContentElement.querySelector('[data-rs-pinned-rows]')
    : null;
}

export function unitForClientPoint(
  clientX,
  clientY,
  axis,
  lanesContentElement,
 ...
 */
export function resourceLaneForClientPoint(
  clientX,
  clientY,
  laneLayout,
  rowsContentElement,
  ...
  if (pinned && pinned.element && pinned.layout && pinned.layout.rows.length) {
    ...
    if (isInsideRect(clientX, clientY, bandRect, orientation)) {
      return pinned.layout.locate(
        offsetWithinRect(clientX, clientY, bandRect, orientation),
      );
    }
  }
  ...
  return laneLayout.locate(
    offsetWithinRect(clientX, clientY, rect, orientation),
  );
}
 ...
 */
export function cellsBetween(laneLayout, startCell, endCell) {
  const flat = [];
  for (const row of laneLayout.rows) {
    for (let lane = 0; lane < row.laneCount; lane++) {
      flat.push({ resourceId: row.resource.id, lane });
    }
  }
  ...
  const indexOf = (cell) =>
    flat.findIndex(
      (candidate) =>
        candidate.resourceId === cell.resourceId &&
        candidate.lane === cell.lane,
    );
